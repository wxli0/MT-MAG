...
def count_train(data_path, folder_path, suffix = "_trimmed.fa"):
    """
    Counts the genome size, contig number, file number in data_path within folder path for test set
...
def count_test(folder_path, suffix = "_trimmed.fa"):
    """
    Counts the genome size, contig number, file number in folder path for test set
    Assuming the fasta file ends with suffix
    """
    genome_size = 0
    contig_num = 0
...
if args.type == "training":
    if args.task == 1 and args.tool  == "MT-MAG":
        ...
        genome_size, contig_num, file_num = count_train(data_path, folder_path)
        ...
        genome_size_11, contig_num_11, file_num_11 = count_train(data_path_11, folder_path_11)
        ...
        genome_size_1, contig_num_1, file_num_1 = count_train(data_path_11, folder_path_11)
        ...
        genome_size_2, contig_num_2, file_num_2 = count_train(data_path_11, folder_path_11)
...
if args.type == "test":
    if args.task == 1 and args.tool == "DeepMicrobes":
        ...
        genome_size, contig_num, file_num = count_test(folder_path)
    elif args.task == 2 and args.tool == "DeepMicrobes":
