...
def build_pipeline(num_features, classifier):
    ...
    dim_reduce_fraction = 0.005 # used to be 0.005 for previous tests
    ...
    if classifier == 'linear-svm':
        ...
        # normalizers.append(('classifier',svm.SVC(kernel='linear', tol=1e-7, probability=True, decision_function_shape ='ovr', max_iter=1000000)))
    ...
    if classifier == 'rbf-svm':
        normalizers.append(('classifier',svm.SVC(kernel='rbf', probability=True)))
    if classifier == 'lda':
